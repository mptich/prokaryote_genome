...
for ordinal, dir1 in enumerate(taxaDict.keys(), start = 1):
    ...
    for dir2, taxa in taxaDict.iteritems():
        dirTaxaTypeDictDict[dir1][taxa.type].append(cogDist[dir1][dir2])
for d in dirTaxaTypeDictDict.values():
    for taxaType, l in d.iteritems():
        d[taxaType] = np.mean(l)
...
for dir in dirTaxaTypeDictDict.keys():
    taxaType, cost = taxaTypeTree.optimal(dirTaxaTypeDictDict[dir])
